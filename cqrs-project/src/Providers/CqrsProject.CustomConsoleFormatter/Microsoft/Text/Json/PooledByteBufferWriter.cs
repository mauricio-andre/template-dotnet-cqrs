...
namespace CqrsProject.CustomConsoleFormatter.Microsoft.Text.Json;

[SuppressMessage("Maintainability", "S112", Justification = "Essa é uma cópia de uma classe interna da microsoft")]
[SuppressMessage("Code Smell", "S1006", Justification = "Essa é uma cópia de uma classe interna da microsoft")]
internal sealed class PooledByteBufferWriter : PipeWriter, IDisposable
{
    ...
    public override void Advance(int bytes)
    {
        ...
        Debug.Assert(_index <= _rentedBuffer.Length - bytes);
        _index += bytes;
    }
            ...
            if ((uint)newSize > MaximumBufferSize)
            {
                ...
                if ((uint)newSize > MaximumBufferSize)
                    throw new OutOfMemoryException(
                        string.Format(
                            "Memory limit of {0} has been reached, current size of {1}",
                            MaximumBufferSize,
                            (uint)newSize));
            }
